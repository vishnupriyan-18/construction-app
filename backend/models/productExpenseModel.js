...
const getProductExpenses = (projectId) =>
  db.prepare('SELECT * FROM product_expenses WHERE project_id = ? ORDER BY expense_date DESC, created_at DESC').all(projectId)

const getTotalProductExpenses = (projectId) => {
  const row = db.prepare('SELECT COALESCE(SUM(amount), 0) as total FROM product_expenses WHERE project_id = ?').get(projectId)
...
const deleteProductExpense = (id, projectId) => {
  const result = db.prepare('DELETE FROM product_expenses WHERE id = ? AND project_id = ?').run(id, projectId)
  return result.changes > 0
}

module.exports = { getProductExpenses, getTotalProductExpenses, createProductExpense, updateProductExpense, deleteProductExpense }
